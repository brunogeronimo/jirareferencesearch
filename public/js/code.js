	...
	function makeTable(data){
		...
		$(data).each(function(index, jira){
			...
			$(jira.referencesStatus).each(function(index, reference){
				...
				texts += reference.key + '-' + reference.status + ':';
				if (reference.assignee !== undefined){
					texts += reference.assignee;
				}else{
					texts += '[UNASSIGNED]';
				}
				texts += ' ';
				if (reference.estimative !== undefined){
					texts += '[' + reference.estimative + ']';
				}
				//texts += '\n';
			});
