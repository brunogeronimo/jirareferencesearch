...
class JiraService
{
	...
	public function search(string $jql = ''){
		...
		foreach ($issues as $issue) {
			...
			if (isset($issue->fields->assignee->key)){
				$aux['assignee'] = $issue->fields->assignee->key;
			}
			...
			if ($reference !== null){
				...
				foreach ($refArray as $ref) {
					try{
							...
							if (isset($jiraSearchResult->fields->assignee->key)){
							...
							if (isset($jiraSearchResult->fields->timetracking->originalEstimate)){
								$referenceInfo['estimative'] = $jiraSearchResult->fields->timetracking->originalEstimate;
							}
